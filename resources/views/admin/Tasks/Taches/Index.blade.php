...
<div class="content">
    @can('task_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route("admin.tasks.create") }}" style="float: right; margin-top: 10px;">
                            ...
                            <tbody>
                                @foreach($tasks as $key => $task)
                                    <tr data-entry-id="{{ $task->id }}">
                                        ...
                                        @can('task_updateS')
                                        <td>
                                            <form action="{{ route('admin.tasks.updateS', $task->id) }}" method="POST" >
                                                @csrf
                                                <div style="position: relative; width: 100%; height: 37px;">
                                                    <select name="value" class="main" required onchange="this.form.submit()">
                                                      @foreach (['On Hold','Completed'] as $c)
                                                        <option {{$task && $task->value == $c?'selected':''}} value="{{$c}}">{{$c}}</option>
                                                      @endforeach 
                                                    </select>
                                                </div>
                                            </form>
                                        ...
                                        <td>
                                            ...
                                            @can('task_delete')
                                                <form class="Formbtn" action="{{ route('admin.tasks.destroy', $task->id) }}" method="POST" style="display: inline-block;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
