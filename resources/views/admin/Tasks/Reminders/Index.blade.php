...
<div class="content">
    @can('reminder_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route("admin.reminders.create") }}" style="float: right; margin-top: 10px;">
                            ...
                            <tbody>
                                @foreach($reminders as $key => $reminder)
                                    <tr data-entry-id="{{ $reminder->id }}">
                                        ...
                                        @can('reminder_updateS')
                                        <td>
                                            <form action="{{ route('admin.reminders.updateS', $reminder->id) }}" method="POST" >
                                                @csrf
                                                <div style="position: relative; width: 100%; height: 37px;">
                                                    <select name="value" class="main" required onchange="this.form.submit()">
                                                      @foreach (['On Hold','Completed'] as $c)
                                                        <option {{$reminder && $reminder->value == $c?'selected':''}} value="{{$c}}">{{$c}}</option>
                                                      @endforeach 
                                                    </select>
                                                </div>
                                            </form>
                                        ...
                                        <td>
                                            ...
                                            @can('reminder_delete')
                                                <form class="Formbtn" action="{{ route('admin.reminders.destroy', $reminder->id) }}" method="POST" style="display: inline-block;">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
...
<script>
    ...
    $('.datatable-Reminder:not(.ajaxTable)').DataTable({ buttons: dtButtons })
