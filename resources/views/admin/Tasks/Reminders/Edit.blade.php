...
<div class="wrapper">
    ...
    <div class="form pt-4">
      <form action="{{ route("admin.reminders.update", [$reminder->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="line">
          ...
          <div class="custom_select">
            <select name="user_id" id="users" class="custom_select" required>
              @foreach ($users as $user)
                <option {{$reminder && $reminder->user_id == $user->id?'selected':''}} value="{{$user->id}}">{{$user->name}}</option>
              @endforeach
            </select>
          </div>
          </div>
